...
impl GuardrailsConfig {
    pub fn load_from_repo(target: &Path) -> Result<(PathBuf, Self)> {
        ...
        let mut config: GuardrailsConfig = toml::from_str(&raw)
            .with_context(|| format!("failed to parse {}", config_path.display()))?;
        if !config.rules.rule_packs.enabled.is_empty() {
            crate::profile::ResolvedProfile::load_from_config(&config)?
                .apply_rule_packs(&mut config)
                .with_context(|| {
                    format!("failed to expand rule packs from {}", config_path.display())
                })?;
        }
        Ok((repo_root, config))
    }
...
pub struct RulesConfig {
    ...
    #[serde(default)]
    pub rule_packs: RulePackSelectionConfig,
    #[serde(default)]
...
pub struct RulePackSelectionConfig {
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Serialize, Deserialize, PartialEq, Eq)]
pub struct EvidenceRequirementConfig {
    #[serde(default)]
    pub name: String,
    #[serde(default)]
    pub changed_paths: Vec<String>,
