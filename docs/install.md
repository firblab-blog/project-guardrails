...
For this phase, the recommended convenience install path is the published
`crates.io` package:
...
If you want a repeatable pinned install, specify the tagged crate version:

```bash
cargo install project-guardrails --locked --version VERSION
```

The public fallback path is a prebuilt binary from a tagged GitHub release.
Use the release archive that matches your platform, verify it against
`SHA256SUMS`, extract it, put the binary on `PATH`, then confirm the binary.
...
- publishing `project-guardrails` on `crates.io`
- installing with `cargo install project-guardrails --locked`
...
## Recommended Path: `crates.io`

For most users, install from `crates.io`:
...
Use a pinned version when you want repeatability in CI or team setup docs:

```bash
cargo install project-guardrails --locked --version VERSION
```

`project-guardrails` is a Rust CLI, so this path assumes Rust and Cargo are
already available on your machine.
If you do not want to install through Cargo, use the tagged release archive
fallback below.
...
developer path from a checkout instead.
...
For public users, the primary documented path is the published `crates.io`
package.
...
- a pushed `v*` tag publishes the matching crate version to `crates.io`
- the same release workflow publishes public release archives from that tag
