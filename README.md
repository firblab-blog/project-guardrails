...
2. run `init`
3. edit the installed repo-local files and create repo-local task state for approved work
4. run `doctor`, `check`, and `pre-work`
5. enable the repo-tracked commit checks locally and wire the same baseline into CI
6. use durable handoffs during work and `upgrade` later to refresh the installed baseline
...
Use `tasks new|claim|update|close` to manage approved work under
