...
The recommended public install path is the `crates.io` package.
...
If you want a pinned install for repeatability, use:

```bash
cargo install project-guardrails --locked --version VERSION
```

If you prefer not to install from `crates.io`, use the tagged GitHub release
archive for your platform and verify it against `SHA256SUMS` before
extracting it.
