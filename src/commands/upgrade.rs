...
pub fn run(args: UpgradeArgs) -> Result<()> {
    ...
    let review_stale_paths = collect_review_stale_paths(&stale_paths, &removable_stale_paths);
    let summary = build_upgrade_summary(
        &repo_root,
        &current,
        &target_profile,
        &target_ci,
...
fn build_upgrade_summary(
    repo_root: &std::path::Path,
    current: &GuardrailsConfig,
    target_profile: &ResolvedProfile,
    target_ci: &str,
    stale_paths: &[String],
    removable_stale_paths: &[String],
    review_stale_paths: &[String],
) -> UpgradeSummary {
    UpgradeSummary {
...
fn collect_review_stale_paths(
    ...
    stale_paths
        .iter()
        .filter(|path| !removable_stale_paths.contains(path))
        .cloned()
        .collect()
}
