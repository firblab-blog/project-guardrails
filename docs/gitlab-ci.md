...
Use Cargo install fallback when:

- no published release asset exists for your runner platform
- you already have Rust and Cargo in the image for other reasons
- you prefer building from the published crate instead of downloading the
  release archive

The Cargo fallback command is:

```bash
cargo install project-guardrails --locked --version 0.1.0 --root "$CI_PROJECT_DIR/.project-guardrails-root"
```

If you use that fallback, keep the same `PATH` export and artifacts pattern.
