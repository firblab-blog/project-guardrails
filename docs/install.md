...
For this phase, the recommended public install path is a prebuilt binary from
...
If `project-guardrails` is published on `crates.io`, that remains an optional
Cargo-based install path:
...
For a repeatable Cargo install, specify the tagged crate version:
...
- optionally publishing `project-guardrails` on `crates.io`
- optionally installing with `cargo install project-guardrails --locked`
...
## Recommended Path: Tagged Release Archives

Choose the tagged GitHub release archive that matches your platform, then
verify it against `SHA256SUMS` before extraction.

## Optional Path: `crates.io`

If the package is published on `crates.io`, you can install it with Cargo:

```bash
cargo install project-guardrails --locked
```

For a pinned Cargo install:

```bash
cargo install project-guardrails --locked --version VERSION
```

This path assumes Rust and Cargo are already available on your machine.
...
- the same GitLab tag pipeline publishes public release archives from that tag
