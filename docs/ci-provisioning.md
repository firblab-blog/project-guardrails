...
- you already provision Rust and Cargo in CI for other jobs
- no published release asset exists for your runner platform
- you explicitly prefer building from the published crate instead of using the
  release archive

The Cargo fallback command is:

```bash
cargo install project-guardrails --locked --version 0.1.0
```
