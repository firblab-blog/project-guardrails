...
cargo run -- pre-work --target /path/to/repo
cargo run -- handoff list --target /path/to/repo
```
...
`pre-work` captures the structured repo summary contributors or agents should
read before substantial work.

For durable repo-local workflow state, use `handoff new|list|close` under
`.guardrails/state/handoffs/`.
Bare `cargo run -- handoff --target /path/to/repo` only prints the installed
...
For V0, use `cargo run -- upgrade --plan --target /path/to/repo` first. Then either:

- run `cargo run -- upgrade --apply --target /path/to/repo` for the thin wrapper path
