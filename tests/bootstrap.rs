...
fn init_materializes_expected_files_for_builtin_profile() {
    ...
    let config =
        fs::read_to_string(repo.join(".guardrails/guardrails.toml")).expect("guardrails config");
    let config: toml::Value = toml::from_str(&config).expect("structured config");
    assert_eq!(
...
#[test]
fn custom_profile_default_rule_pack_expands_into_installed_config_and_templates() {
    let temp = TempDir::new().expect("temp dir");
    let repo = temp.path().join("rule-pack-defaults");
    copy_dir(&fixture_root().join("bare-repo"), &repo);

    let profile_dir = temp.path().join("rule-pack-profile");
    fs::create_dir_all(profile_dir.join("templates/docs/project")).expect("profile docs");
    fs::create_dir_all(profile_dir.join("templates/tools")).expect("profile files");
    fs::create_dir_all(profile_dir.join("assets/rules/semgrep")).expect("semgrep asset");
    fs::create_dir_all(profile_dir.join("assets/policy/demo")).expect("conftest asset");
    fs::write(
        profile_dir.join("profile.toml"),
        r#"schema_version = 1
name = "rule-pack-demo"
description = "Rule pack config surface test profile"
default_ci = "none"
docs_enabled = true
required_docs = []
...
[rule_packs]
default_enabled = ["demo-pack"]

[[rule_packs.available]]
name = "demo-pack"
description = "Tiny fixture pack that maps to existing enforcement surfaces."
severity = "advisory"
required_docs = ["docs/project/demo-pack.md"]
required_files = ["tools/demo-pack-note.txt"]
semgrep_config_paths = ["rules/semgrep/demo.yml"]
conftest_policy_paths = ["policy/demo"]

[[rule_packs.available.link_requirements]]
changed_paths = ["src/"]
required_docs = ["docs/project/demo-pack.md"]
message = "demo pack requires its doc when src changes"

[[rule_packs.available.forbidden_patterns]]
pattern = "DEMO_ONLY"
message = "demo pack placeholder must not be committed"
...
"#,
    ...
    .expect("profile");
    fs::write(
        profile_dir.join("templates/docs/project/demo-pack.md"),
        "# Demo Pack\n",
    )
    .expect("demo doc template");
    fs::write(
        profile_dir.join("templates/tools/demo-pack-note.txt"),
        "demo pack note\n",
    )
    .expect("demo required file template");
    fs::write(
        profile_dir.join("assets/rules/semgrep/demo.yml"),
        "rules: []\n",
    )
    .expect("semgrep asset");
    fs::write(
        profile_dir.join("assets/policy/demo/example.rego"),
        "package demo\n",
    )
    .expect("conftest asset");
    ...
    assert!(repo.join("docs/project/demo-pack.md").exists());
    assert!(repo.join("tools/demo-pack-note.txt").exists());
    assert!(repo.join("rules/semgrep/demo.yml").exists());
    assert!(repo.join("policy/demo/example.rego").exists());

    let config =
        fs::read_to_string(repo.join(".guardrails/guardrails.toml")).expect("guardrails config");
    assert!(config.contains("enabled = [\"demo-pack\"]"));
    assert!(config.contains("required = [\"docs/project/demo-pack.md\"]"));
    assert!(config.contains("required_files = ["));
    assert!(config.contains("\"tools/demo-pack-note.txt\""));
    assert!(config.contains("[[rules.link_requirements]]"));
    assert!(config.contains("demo pack requires its doc when src changes"));
    assert!(config.contains("[[rules.forbidden_patterns]]"));
    assert!(config.contains("pattern = \"DEMO_ONLY\""));
    assert!(config.contains("[engines.semgrep]"));
    assert!(config.contains("enabled = true"));
    assert!(config.contains("config_paths = [\"rules/semgrep/demo.yml\"]"));
    assert!(config.contains("policy_paths = [\"policy/demo\"]"));

    let profile_lock =
        fs::read_to_string(repo.join(".guardrails/profile.lock")).expect("profile lock");
    assert!(profile_lock.contains("path = \"docs/project/demo-pack.md\""));
    assert!(profile_lock.contains("path = \"tools/demo-pack-note.txt\""));
}
...
fn init_guardrails_profile_installs_seeded_doctrine_docs() {
    ...
    let claude = fs::read_to_string(repo.join("CLAUDE.md")).expect("claude adapter");
    assert!(claude.contains("Treat `AGENTS.md` as the canonical repo guidance."));
    assert!(claude.contains("project-guardrails brief --target ."));
    assert!(claude.contains("<!-- guardrails:managed start id=adapter-context"));

    let cursor = fs::read_to_string(repo.join(".cursor/rules/project-guardrails.mdc"))
        .expect("cursor adapter");
    assert!(cursor.contains("alwaysApply: true"));
    assert!(cursor.contains("Treat `AGENTS.md` as the canonical repo guidance."));
    assert!(cursor.contains("project-guardrails brief --target ."));
    assert!(cursor.contains("<!-- guardrails:managed start id=adapter-context"));
    ...
    assert!(profile_lock.contains("path = \"CLAUDE.md\""));
    assert!(profile_lock.contains("path = \".cursor/rules/project-guardrails.mdc\""));
    assert!(profile_lock.contains("id = \"adapter-context\""));
}
...
fn init_supports_custom_profile_paths_and_external_engines() {
    ...
    assert!(config.contains("installed_by_version = \"0.2.0\""));
...
#[test]
fn upgrade_apply_materializes_guardrails_adapter_targets() {
    let temp = TempDir::new().expect("temp dir");
    let repo = temp.path().join("upgrade-guardrails-adapters");
    copy_dir(&fixture_root().join("bare-repo"), &repo);
    ...
    let claude = fs::read_to_string(repo.join("CLAUDE.md")).expect("claude adapter");
    assert!(claude.contains("Claude-specific entry point"));
    assert!(claude.contains("<!-- guardrails:managed start id=adapter-context"));

    let cursor = fs::read_to_string(repo.join(".cursor/rules/project-guardrails.mdc"))
        .expect("cursor adapter");
    assert!(cursor.contains("Cursor-specific entry point"));
    assert!(cursor.contains("<!-- guardrails:managed start id=adapter-context"));
...
#[test]
fn pre_commit_expands_enabled_rule_pack_from_repo_config() {
    let temp = TempDir::new().expect("temp dir");
    let repo = temp.path().join("pre-commit-rule-pack");
    copy_dir(&fixture_root().join("bare-repo"), &repo);

    let profile_dir = temp.path().join("pre-commit-rule-pack-profile");
    fs::create_dir_all(&profile_dir).expect("profile dir");
    fs::write(
        profile_dir.join("profile.toml"),
        r#"schema_version = 1
name = "pre-commit-rule-pack"
description = "Rule pack pre-commit test profile"
default_ci = "none"
docs_enabled = true
required_docs = ["docs/project/implementation-tracker.md"]
required_files = ["README.md", "AGENTS.md", ".guardrails/guardrails.toml"]
...
[[rule_packs.available]]
name = "commit-pack"
description = "Maps to existing pre-commit checks."
severity = "error"

[[rule_packs.available.link_requirements]]
changed_paths = ["src/"]
required_docs = ["docs/project/implementation-tracker.md"]
message = "commit pack requires tracker updates"

[[rule_packs.available.forbidden_patterns]]
pattern = "console\\.log"
message = "commit pack rejects debug logging"
...
"#,
    ...
    let config_path = repo.join(".guardrails/guardrails.toml");
    let config = fs::read_to_string(&config_path).expect("read config");
    fs::write(
        &config_path,
        config.replace("enabled = []", "enabled = [\"commit-pack\"]"),
    )
    .expect("enable pack");
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("[commit_link_requirement_missing]"));
    assert!(stderr.contains("commit pack requires tracker updates"));
    assert!(stderr.contains("[commit_forbidden_pattern]"));
    assert!(stderr.contains("commit pack rejects debug logging"));
}
...
#[test]
fn doctor_and_check_fail_when_guardrails_cursor_adapter_is_missing() {
    let temp = TempDir::new().expect("temp dir");
    let repo = temp.path().join("missing-cursor-adapter");
    copy_dir(&fixture_root().join("bare-repo"), &repo);
    ...
    fs::remove_file(repo.join(".cursor/rules/project-guardrails.mdc"))
        .expect("remove cursor adapter");

    let doctor = run_guardrails_expect_failure(&[
        "doctor",
        "--target",
        repo.to_str().unwrap(),
        "--format",
    ...
    let json: Value = serde_json::from_slice(&doctor.stdout).expect("valid doctor json");
    assert_missing_cursor_adapter(&json);

    let check = run_guardrails_expect_failure(&[
        "check",
        "--target",
        repo.to_str().unwrap(),
        "--format",
    ...
    let json: Value = serde_json::from_slice(&check.stdout).expect("valid check json");
    assert_missing_cursor_adapter(&json);
}

fn assert_missing_cursor_adapter(json: &Value) {
    assert!(
        json["diagnostics"]
            .as_array()
            .expect("diagnostics")
            .iter()
            .any(|diagnostic| {
                diagnostic["code"] == "required_file_missing"
                    && diagnostic["message"]
                        .as_str()
                        .expect("message")
                        .contains(".cursor/rules/project-guardrails.mdc")
            })
    );
}
