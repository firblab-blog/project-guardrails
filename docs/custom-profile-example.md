...
This repo ships three built-in public profiles:
...
- profile-owned enforcement and freshness defaults in `profile.toml`
...
- installs `.pre-commit-config.yaml` so repo-tracked commit checks are part of
  the baseline
...
- requires task references in commit messages
- adds a path-to-doc link requirement for staged `src/` changes
- forbids a placeholder diff pattern before commit
- declares starter-content markers for its custom `AGENTS.md` and release
  checklist templates
- refreshes managed context blocks inside `AGENTS.md` and the shared
  implementation tracker template
- defaults CI to GitLab without changing Rust code

The example's `link_requirements.changed_paths = ["src/"]` entry is deliberate
example doctrine, not a global runtime assumption.
Custom profiles should adapt those path rules to the consumer repo's actual
layout.
...
- declare mechanical enforcement defaults in the profile
- add starter-content and managed-block behavior without changing the CLI
