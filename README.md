...
For `v0.1`, the only supported public install path is:
...
The intended install order once the packaging work is fully live is:
...
Today, the release archive is the real install contract.
...
If a release does not contain an archive for your platform, there is not yet a
separate public package-manager path to fall back to.
The remaining path is a developer build from a source checkout.
