...
fn init_help_mentions_default_profile_and_builtin_summaries() {
    ...
    assert!(stdout.contains("`minimal` is the default happy path"));
    assert!(stdout.contains("Built-in profiles:"));
    assert!(stdout.contains("minimal      Smallest cross-language starting point."));
    assert!(stdout.contains("docs-driven  Everything in minimal, plus a required decision log."));
...
fn init_dry_run_separates_edit_first_run_next_and_planned_files() {
    ...
    assert!(stdout.contains("profile_choice: default starting point"));
...
fn init_success_output_recommends_decision_log_for_docs_driven_profile() {
    ...
    assert!(stdout.contains(
        "Why this profile: use this when you want the minimal baseline plus a required decision log"
    ));
...
fn init_supports_custom_profile_paths_and_external_engines() {
    ...
    assert!(config.contains("installed_by_version = \"0.1.13\""));
...
fn init_preserves_existing_repo_files_and_reports_kept_existing_paths() {
    ...
    assert_eq!(
        fs::read_to_string(repo.join("docs/project/implementation-tracker.md"))
            .expect("tracker contents"),
        "# Existing Tracker\n\n- keep the current tracker content\n"
    );
    assert_eq!(
